...
describe("pasteNeedsPrompt", () => {
  it("asks when the target span already holds notes", () => {
    const score = melodyScore();
    expect(pasteNeedsPrompt(score, score.tracks[0].id, 0, score.ppq)).toBe(
      true,
    );
  });
  ...
  it("does not ask when pasting into empty time", () => {
    ...
    expect(
      pasteNeedsPrompt(score, score.tracks[0].id, emptyTick, score.ppq),
    ).toBe(false);
  });

  it("does not ask about a track with nothing on it", () => {
    const score = melodyScore();
    expect(pasteNeedsPrompt(score, score.tracks[1].id, 0, score.ppq)).toBe(
      false,
    );
  });
