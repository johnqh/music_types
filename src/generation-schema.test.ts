...
describe("generateScoreRequestSchema", () => {
  ...
  it("rejects an invalid clef", () => {
    const request = {
      ...validGenerateRequest(),
      tracks: [
        { name: "X", instrumentName: "X", midiProgram: 0, clef: "soprano" },
      ],
    };
    expect(generateScoreRequestSchema.safeParse(request).success).toBe(false);
  });
...
function validRegenerateRequest(): RegenerateRegionRequest {
  ...
  return {
    ...
    range,
    precedingContext: extractFragment(score, {
      startTick: 0,
      endTick: measureTicks,
      trackIds: [track.id],
    }),
    selectedFragment: extractFragment(score, range),
    followingContext: extractFragment(score, {
      startTick: 2 * measureTicks,
      endTick: 3 * measureTicks,
      trackIds: [track.id],
    }),
    constraints: {
...
describe("regenerateRegionRequestSchema", () => {
  it("accepts a valid request", () => {
    expect(
      regenerateRegionRequestSchema.safeParse(validRegenerateRequest()).success,
    ).toBe(true);
  });
  ...
  it("accepts a request with optional constraint fields populated", () => {
    const request: RegenerateRegionRequest = {
      ...
      constraints: {
        ...
        maximumPolyphony: 1,
        allowedPitchRangeByTrack: {
          "track-1": { lowestMidi: 40, highestMidi: 80 },
        },
      },
    };
  ...
  it("rejects a candidateCount of 0", () => {
    ...
    expect(regenerateRegionRequestSchema.safeParse(request).success).toBe(
      false,
    );
  });
  ...
  it("rejects preserveMeasureCount: false (constraints are always preserve=true per spec §12)", () => {
    ...
    const invalid = {
      ...request,
      constraints: { ...request.constraints, preserveMeasureCount: false },
    };
    expect(regenerateRegionRequestSchema.safeParse(invalid).success).toBe(
      false,
    );
  });
...
describe("regenerateRegionResultSchema", () => {
  it("accepts a candidates + warnings result", () => {
    ...
    const result = {
      candidates: [
        { id: "c1", label: "Variation 1", fragment: request.selectedFragment },
      ],
      warnings: [],
    };
...
describe("createGenerationJobRequestSchema", () => {
  ...
  it("accepts each of the five kinds", () => {
    for (const kind of [
      ...
      expect(
        createGenerationJobRequestSchema.safeParse({ ...valid(), kind })
          .success,
      ).toBe(true);
    }
  });
  ...
  it("rejects an unknown kind", () => {
    expect(
      createGenerationJobRequestSchema.safeParse({
        ...valid(),
        kind: "transcribe",
      }).success,
    ).toBe(false);
  ...
  it("requires a projectId, since every job now owns one", () => {
    ...
    expect(createGenerationJobRequestSchema.safeParse(rest).success).toBe(
      false,
    );
  });
});
...
describe("generationJobSchema", () => {
  ...
  it("accepts queued, which is a job waiting its turn", () => {
    expect(
      generationJobSchema.safeParse({ ...valid(), status: "queued" }).success,
    ).toBe(true);
  });

  it("rejects a status outside the five", () => {
    expect(
      generationJobSchema.safeParse({ ...valid(), status: "paused" }).success,
    ).toBe(false);
  });
...
describe("regenerationConstraintsSchema preserveMeasureCount", () => {
  const base = () =>
    ({ preserveTimeSignatures: true, preserveTempoEvents: true }) as const;
  ...
  it("still accepts it when present", () => {
    expect(
      regenerationConstraintsSchema.safeParse({
        ...base(),
        preserveMeasureCount: true,
      }).success,
    ).toBe(true);
  ...
  it("keeps time signatures and tempo events mandatory regardless of alignment", () => {
    expect(
      regenerationConstraintsSchema.safeParse({ preserveTempoEvents: true })
        .success,
    ).toBe(false);
    expect(
      regenerationConstraintsSchema.safeParse({ preserveTimeSignatures: true })
        .success,
    ).toBe(false);
  });
});
