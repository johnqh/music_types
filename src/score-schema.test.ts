...
describe("parseScore", () => {
  ...
  it("accepts a note event with velocity/midi fields at the edges of their valid ranges", () => {
    const score = createEmptyScore({
      title: "Edges",
      measures: 1,
      tracks: [{ name: "Piano", midiProgram: 127, midiChannel: 15 }],
    });
  ...
  it("rejects a track with midiProgram above 127", () => {
    ...
    const bad = {
      ...score,
      tracks: [{ ...score.tracks[0], midiProgram: 128 }],
    };
    expect(() => parseScore(bad)).toThrow(ZodError);
  });
...
/*
  `any` on purpose: this helper builds *deliberately malformed* scores to feed
  the schema, so typing its inputs would defeat the tests that use it.

  The directive sits on each parameter rather than above the function, because
  `disable-next-line` covers exactly one row — placed above `function` it
  suppressed nothing and then reported itself as unused, which is what it was
  doing before.
*/
function withSingleNoteEvent(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  score: any,
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  track: any,
  noteOverrides: Record<string, unknown>,
  pitchOverrides: Record<string, unknown> = {},
) {
  const measure = track.measures[0];
  const voiceId = measure.voices[0].id;
  return {
